...
- **Market, shop** (`worker/shop.ts`, `src/market/shop.ts`, `src/market/ShopTab.tsx`, `#/market/shop`): members spend coins on a pack from a set they pick; the wrapper is still random. Sets are listed by tier, cheapest first. Packs cost a set price per set tier (`TIER_FLOOR`: 2,500 / 7,500 / 20,000 / 50,000 coins), so buying one is a treat. A few sets' cards are worth nearly that or more, so a pack never costs less than its cards' average value times `PACK_MARKUP` (1.4), and buying packs to resell can't make coins. `npm run pack-prices` (`scripts/packPrices.ts`) works the values out: it opens 3,000 packs of each openable set with the real engine, prices every card that came out in its finish, and writes `src/market/packPrices.json`; sets missing from it aren't sold. After changing the pricing in `shop.ts`, `npm run pack-prices -- --reprice` re-applies it to the saved values without fetching prices. A bought pack is rolled when it's bought (`rollSet` in `worker/packs.ts`, shared with free packs) and waits, unopened, in `pack_inventory` (migration 0009) with an `s-` id, up to 50 at a time. The coins come off and the pack goes in in one batch, which the balance's `CHECK` rolls back if it can't cover it.
