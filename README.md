...
- **Worker** (`worker/`): `/api/auth/*` (guest, register, login, logout, password, Google OAuth with PKCE), `/api/packs/deal` and `/api/collection` (push changes, pull changes since a cursor). Everything outside `/api/` is served straight from the built app (`run_worker_first` in `wrangler.jsonc`).
